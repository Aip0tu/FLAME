...
from FLAME_plugin.args import PredictArgs, TrainArgs
from FLAME_plugin.models.utils import load_args, load_checkpoint, load_scalers, makedirs, timeit, update_prediction_args
from FLAME_plugin.dataprocess.data import get_data, get_data_from_smiles, MoleculeDataLoader, MoleculeDataset
from FLAME_plugin.dataprocess.features import set_extra_atom_fdim, set_extra_bond_fdim
...
def flam_predict(target, smiles=None) -> None:
    ...
    model_path = f'{pwd}/model_checkpoint/{target}.pt'
    output_file = f'{pwd}/model_checkpoint/tmp_out.csv'
    md5_dict = {
    ...
    if os.path.exists(model_path):
        if get_md5_checksum(model_path)!=md5_dict[target]:
            print('model maybe different from the original version')
    else:
        print(f'Downloading Model {target}.pt From Google Drive...')
        gdown.download('https://drive.google.com/uc?id='+gd_dict[target], model_path, quiet=False)
        if get_md5_checksum(model_path)!=md5_dict[target]:
            raise ValueError(f'model {target} download failed!')

    pred_args = ['--test_path', '', '--checkpoint_path', model_path,
